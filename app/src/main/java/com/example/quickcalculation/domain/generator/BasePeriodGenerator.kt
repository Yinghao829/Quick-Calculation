...
object BasePeriodGenerator {
    ...
    fun generate(difficulty: Difficulty, random: Random): Question {
        ...
        return if (random.nextBoolean()) {
            ...
            val stem = "${year}年${topic.name}为${NumberUtil.format(b, dec)}${topic.unit}，同比${NumberUtil.formatTrend(r)}，求${year + 1}年${topic.name}。"
            val op = if (r < 0) "−" else "+"
            val rateLabel = if (r < 0) "下降率" else "增长率"
            val explanation = "现期量 = 基期量 × (1 ${op} ${rateLabel}) = ${NumberUtil.format(b, dec)} × ${NumberUtil.formatFactor(r)} = ${NumberUtil.format(answer, dec)}${topic.unit}"
            Question(
